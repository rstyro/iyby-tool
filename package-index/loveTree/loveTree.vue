      ...
      this.stopBackgroundMusic();
		...
		methods: {
			...
			startTypewriter() {
				...
				// 初始化第一行
				if (this.params.textList.length > 0) {
			...
			typeNextCharacter() {
				if (this.currentLineIndex >= this.params.textList.length) {
					// 所有行打字完成
					this.typing = false;
				...
				const currentLine = this.params.textList[this.currentLineIndex];
			...
			async flowerAnimation2() {
				do {
					this.tree.flower(3) // 每次开放更多花朵
					await this.delay(30)
			...
			// 推荐使用这个综合优化版本
			async flowerAnimation1() {
				console.log('开始花朵开放动画 - 性能优化版');

				const isWeChatEnv = typeof wx !== 'undefined';
				const batchSize = isWeChatEnv ? 1 : 2; // 微信环境更保守
				const delayTime = isWeChatEnv ? 40 : 25;
			...
			// 修改页面的 flowerAnimation 方法
			async flowerAnimation() {
				console.log('开始花朵开放动画');
				// H5环境：保持原来的分批开放，保证动画效果
				const batchSize = 3;
				const delayTime = 25;
				let frameCount = 0;
				do {
			...
			// ========== 花瓣动画 流畅无残留 ==========
			// 修改页面的 petalAnimation 方法
			async petalAnimation() {
				console.log('开始花瓣动画');
				if (this.tree.isH5) {
					// H5环境：保持原版流畅效果
					console.log('H5：保持原版花瓣动画');
